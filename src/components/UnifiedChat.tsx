...
const UnifiedChat = ({ selectedTool, onSendMessage, userName = "você" }: UnifiedChatProps) => {
  ...
  return (
    <div className="flex-1 flex flex-col h-[calc(100vh-64px)] bg-[hsl(0,0%,4%)]">
      {/* Chat Area */}
          ...
          <AnimatePresence mode="wait">
            {messages.length === 0 ? (
              <motion.div
                ...
                exit={{ opacity: 0, y: -10 }}
                className="flex flex-col items-center justify-center min-h-[50vh] pt-12"
              >
                {/* Welcome */}
                <motion.div
                ...
                <motion.div
                  ...
                  className="w-full max-w-2xl bg-gradient-to-br from-white/[0.03] to-white/[0.01] border border-white/[0.06] rounded-2xl p-6 mb-8"
                >
                  <div className="flex items-start gap-4 mb-6">
                    <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-orange-500/20 to-amber-500/10 border border-orange-500/20 flex items-center justify-center shrink-0">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {tool.examples.map((example) => (
                      <button
                        ...
                        className="px-4 py-2.5 text-sm bg-white/[0.03] hover:bg-white/[0.06] border border-white/[0.06] hover:border-white/[0.12] rounded-xl text-white/60 hover:text-white transition-all"
                      >
                        {example}
                      </button>
                    ))}
              ...
              >
                {messages.map((message, index) => (
                  ...
                  >
                    ...
                    {message.role === "user" && (
                      <div className="w-10 h-10 rounded-xl bg-white/[0.06] border border-white/[0.08] flex items-center justify-center shrink-0 text-white/60 font-semibold text-sm">
          ...
          <form onSubmit={handleSubmit}>
            <div className="relative flex items-center bg-[hsl(0,0%,8%)] border border-white/[0.08] rounded-2xl focus-within:border-orange-500/30 focus-within:ring-4 focus-within:ring-orange-500/5 transition-all">
              ...
              <div className="flex items-center pl-2">
                <button
                  type="button"
                  className="p-2.5 text-white/30 hover:text-white/50 rounded-lg hover:bg-white/[0.04] transition-colors"
                >
              ...
              <input
                ...
                className="flex-1 py-4 px-3 bg-transparent text-white placeholder:text-white/30 text-sm outline-none"
                disabled={isLoading}
              />
              ...
              <div className="flex items-center gap-1 pr-2">
                ...
                <button
                  ...
                  className="w-11 h-11 rounded-xl bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 disabled:opacity-30 disabled:hover:from-orange-500 disabled:hover:to-orange-600 flex items-center justify-center transition-all shadow-lg shadow-orange-500/20"
                >
